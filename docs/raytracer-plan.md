...
#### J.5：自适应采样、firefly 与焦散专项（候选评估完成并停止，2026-07-19）

在 J.4 稳定节点上评估了一个最小自适应采样候选：Preview 保持最大 `32 spp`，以 Film 亮度均值/方差估计相对误差，最小 `8 spp` 后允许稳定像素停止，并扩张四邻域采样 mask 以保护边缘。该候选未修改无偏 Film、路径积分器或 J.4 transmission 重组。

运行与图像证据：

- 完整 Preview 正常结束，compute 为 `185.154s`；候选执行 `779194` 次、跳过 `400454` 次计划像素采样，约减少 `34.0%` 的后续 sample；
- 用户人工确认噪点细微增加，主要位于墙顶面及墙与天空的交线；玻璃轮廓保持稳定；
- 相对 J.4 基线，左侧墙天边界相邻亮度差 P90 增加约 `33.26%`、Laplacian 中位数增加约 `7.93%`；
- 右侧墙天边界相邻亮度差 P90 增加约 `21.78%`、Laplacian 中位数增加约 `8.44%`；
- 玻璃核心相邻亮度差 P90 增加约 `18.60%`、Laplacian 中位数增加约 `37.62%`；
- 区域平均亮度保持稳定，但固定最大预算下的边缘与玻璃颗粒指标明确回归。

结论与处置：

- 当前基于相对误差和四邻域扩张的自适应候选收益不足，已完整回退；Preview 恢复 J.4 的均匀 `32 spp`，不继续调阈值或追加试错；
